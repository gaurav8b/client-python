...
class MyModeler:
    ...
    def save_results(self):
        ...
        returns_df = pd.DataFrame({
            ...
            'net_returns': self.netrets.values(),
            'gross_returns': self.grossrets.values()
        })
        ...
        for date in self.datadict.keys():
            ...
            if forecast is not None:
                ...
                if 'close' in actual.columns:
                    # Calculate Sharpe ratio only if 'close' column exists
                    daily_sharpe = sharpe(actual['close'])
                else:
                    logger.error("Error: 'close' column is missing in the DataFrame")
                    daily_sharpe = np.nan  # Handle missing column gracefully
        ...
        overall_performance = {
            ...
            'total_net_return': sum(self.netrets.values()),
            'total_gross_return': sum(self.grossrets.values()),
            'sharpe_ratio': sharpe(pd.Series(self.netrets.values())),
            'win_rate': np.mean([r > 0 for r in self.netrets.values()]),
            'num_trades': len(self.netrets)
        }
...
mymod.pred_span(do_pl=True, beg_day='2023-12-01', end_day='2023-12-05', tolerance=0.05, tcosts=0.01)

# Print overall performance metrics
overall_performance = pd.read_csv(os.path.join(mymod.output_dir, "overall_performance.csv"))
