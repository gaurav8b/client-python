...
# ... rest of your code
    ...
    if prediction_day not in crypto_df.index:
        logger.error(f"No data available for the specified day: {prediction_day}")
        sys.exit(1)
    ...
    logger.info(f"Predicting for: {prediction_day}")

    historical_fcast, fcast_mape = mymod.day_predict(theday=prediction_day)

    if historical_fcast is not None and fcast_mape is not None:
        print(f"Forecast MAPE for {prediction_day}: {fcast_mape}")

        mymod.do_pl(prediction_day, tolerance=0.001, tcosts=0.00005)
    ...
    specific_day_data = crypto_df[crypto_df.index.normalize() == pd.to_datetime(prediction_day)]
    print(f"Data points for prediction day ({prediction_day}): {len(specific_day_data)}")