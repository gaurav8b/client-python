...
class MyModeler:
    ...
    def init_model(self):
        try:
            ...
            #self.model = self.modeltype.load(path=self.model_file, weights_only=True)
            #self.model = self.modeltype.load(path=self.model_file)
...
def fetch_crypto_data(coin_id, days=365, vs_currency='usd'):
    ...
    df_resampled = df['log_price'].resample('1min').interpolate(method='time').fillna(method='ffill')
...
if __name__ == "__main__":
    ...
    #mymod.fit_span(beg_day=2024-10-01, end_day=2024-10-05 - timedelta(minutes=1))

    # Corrected date format for beg_day and end_day
    mymod.fit_span(
        beg_day=datetime(2024, 10, 1),
        end_day=datetime(2024, 10, 3) - timedelta(minutes=1)
    )
        
    # Select the last available day for prediction
    prediction_day = crypto_df.loc[train_end_date:].index.normalize().unique()
    if len(prediction_day) == 0:
        logger.error("No available days for prediction. Exiting.")
        exit()

    # Choose the last day in the dataset for prediction
    #specific_day = prediction_day[-1]
