...
# ... rest of your code
    ...
    # Find the nearest day with complete data
    complete_data_days = crypto_df.index.normalize().unique()
    closest_day = complete_data_days[np.argmin(np.abs(complete_data_days - prediction_day))]

    # If the closest day is before prediction_day, use the next day with complete data
    if closest_day < prediction_day:
        closest_day = complete_data_days[np.argmin(np.abs(complete_data_days - prediction_day)) + 1]
